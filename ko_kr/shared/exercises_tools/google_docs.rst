...
Studio 또는 OLX를 사용하여 강좌에서 구글 드라이브 파일을 사용할 수 있다.

.. _Enable Google Drive Files in edX Studio:

Studio에서 구글 드라이브 파일 활용하기
***************************************
...
구성요소를 추가하기 전에, :ref:`Enable Google Drive Files in edX Studio` 하는지 확인한다.
...
.. note:: 
  K-MOOC 학습 관리 시스템은 구글 드라이브 파일에 대한 높이 및 너비 값을 설정한다. 이러한 속성을 추가하면, LMS는 변경 내용을 무시한다.
