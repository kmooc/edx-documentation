...
강좌 운영팀은 Studio에서 HTML을 사용하여 **강좌 개요** 페이지에 들어갈 구체적인 내용을 입력해야 한다. 
...
#. 동영상 영역 아래에, YouTube video ID (위 1단계에서 복사한 코드)를 입력한다. 코드를 추가하면, 해당 동영상이 동영상 영역에 나타날 것이다. 
...
학습자가 강좌를 수강하기 전에, 필요한 사전 지식을 충분히 갖추었는지 교수자가 확인하고 싶은 경우가 있다. 이에 강좌 개요에서 일반적인 학습 권장 사항을 제안할 뿐만 아니라, 학습자가 특정 선수 강좌를 수료하거나 사전 시험을 통과한 후에만 강좌를 수강할 수 있도록 설정할 수 있다.
...
학습자가 사전 평가에 있는 오류를 발견할 경우, 교수자 및 강좌 운영팀이 수정을 안 할 수 없다. 이 경우 개별 학습자의 채점을 위해 몇 가지 선택 사항이 있다. 이는 교수자 대시보드에서 이용할 수 있다.
