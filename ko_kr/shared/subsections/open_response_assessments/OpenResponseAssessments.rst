...
개방형 응답 평가에서 교수자는 정답이 정해져있지 않은 문제를 제시하기도 한다. 학습자가 이에 대해 답변을 제출하면 학습자 자신과 동료 학습자가 해당 답변과 강좌 운영팀이 작성한 채점 기준표(rubric)를 비교한다. 학습자는 텍스트 형식의 답변을 제출하는 것이 통상적이나 해당 텍스트에 이미지를 첨부하여 업로드할 수도 있다.
